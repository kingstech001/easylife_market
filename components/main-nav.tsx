"use client"

import type React from "react"
import Link from "next/link"
import { usePathname } from "next/navigation"

import { cn } from "@/lib/utils"
import Image from "next/image"

export function MainNav({ className, ...props }: React.HTMLAttributes<HTMLElement>) {
  const pathname = usePathname()

  const links = [
    { href: "/", label: "Home" },
    { href: "/stores", label: "Stores" },
    { href: "/allStorProducts", label: "products" },
    { href: "/about", label: "About" },
  ]

  return (
    <div className="flex gap-6 md:gap-10 items-center">
      <Link href="/" className="inline-flex items-center group">
        <Image alt="EasyLife Logo" src={"/logo.png"} width={70} height={70} className="drop-shadow-lg"/>
        <div className="">
          <span className="hidden md:block text-xl md:font-bold bg-gradient-to-r from-[#e1a200] via-[#e1a200] to-[#e1a200] bg-clip-text text-transparent">
            EasyLife
          </span>
        </div>
      </Link>
      <nav className={cn("md:flex items-center space-x-4 lg:space-x-6 hidden", className)} {...props}>
        {links.map((link) => {
          const isActive = pathname === link.href
          return (
            <Link
              key={link.href}
              href={link.href}
              className={cn(
                "text-sm transition-colors hover:text-[#e1a200]",
                isActive
                  ? "text-[#e1a200] font-semibold"
                  : "text-muted-foreground"
              )}
            >
              {link.label}
            </Link>
          )
        })}
      </nav>
    </div>
  )
}
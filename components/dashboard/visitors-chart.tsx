    ...
    return (
      <div className="bg-card border border-border rounded-lg shadow-lg p-3 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">{payload[0].value.toLocaleString()}</span>
        </div>
      </div>
    )
  }
...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center h-[320px] text-center space-y-3">
    ...
    <div className="space-y-1">
      <h3 className="font-medium text-foreground">No visitor data yet</h3>
      <p className="text-sm text-muted-foreground max-w-sm">
        Visitor analytics will appear here once traffic starts flowing to your site.
      </p>
    </div>
  </div>
)
  ...
  useEffect(() => {
    ...
    const fetchVisitors = async () => {
      try {
        ...
        const apiUrl =
          user.role === "admin"
            ? "/api/dashboard/admin/analytics/visitors?days=30"
            : "/api/dashboard/seller/analytics/visitors?days=30"
        ...
        const analyticsData = json.analytics?.dailyBreakdown || []
